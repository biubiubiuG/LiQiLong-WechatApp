<template>
	<view class="category-wrap" :data-theme='theme()' :class="theme() || ''">
		...
		<view class="category-content">
			<!--一级分类 大图-->
			<view class="cotegory-type cotegory-type-1" v-if="showType==10">
			...
			<!--一级分类 小图-->
			<view class="cotegory-type cotegory-type-2" v-if="showType==20">
				<scroll-view scroll-y="true" class="scroll-Y" :style="'height:'+scrollviewHigh+'px;'">
					<view class="list">
						<view class="item" v-for="(item,index) in listData" :key="index"
							@click="gotoList(item.categoryId)">
			...
			<!--二级分类-->
			<view class="cotegory-type cotegory-type-3" v-if="showType==30">
