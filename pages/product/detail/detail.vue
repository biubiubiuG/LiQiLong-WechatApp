<template>
	<view class="product-detail pr" :data-theme='theme()' :class="theme() || ''">
		...
		<scroll-view scroll-y="true" class="scroll-Y scroll-box" :style="'height:' + scrollviewHigh + 'px'"
				...
				<swiper class="swiper" indicator-active-color="#ffffff" indicator-color='rgba(255,255,255,.3)'
					...
					<swiper-item v-if="detail.videoId != 0">
						...
						<image v-if="!isVideoPlay"
							:src="detail.poster?detail.poster.filePath:detail.image[0].filePath" mode=""
							@click="isVideoPlay=true"></image>
						<video v-if="isVideoPlay" :src="detail.video.filePath" :autoplay="isVideoPlay"
			...
			<template v-if="ispresale">
				...
				<view class="limited-spike m-0-20">
					<view class="d-s-c mb16">
						<view class="f32">￥<text class="f42 fb">{{subPrice(detail.advance.sku[0].product_price,'1')}}.</text><text
								class="f24 fb">{{subPrice(detail.advance.sku[0].product_price,'2')}}</text></view>
						<view class="tips-box">预估到手价￥{{(detail.advance.sku[0].product_price*1 - detail.advance.sku[0].advance_price*1).toFixed(2)}}
						</view>
					</view>
					<text class="left-name">定金¥{{detail.advance.money}} (尾款立减¥{{detail.advance.sku[0].advance_price}})</text>
					<view class="right-time">
						<view class="d-e-c mb16">预售<image class="jiantou" src="/static/icon/jiantou-white.png" mode="">
							</image>
						</view>
						<Countdown ref="countdown"
							:config="{startstamp: detail.advance.start_time,endstamp: detail.advance.end_time}"
				...
				<view class="bg-white  m-0-20 mb20 p20 mt-down-box">
					...
					<view class="product-presale">
						<view class="d-b-s">
							...
							<view class="flex-1">
								<view class="f24 redF11">￥{{(detail.advance.sku[0].product_price*1 - detail.advance.sku[0].advance_price*1).toFixed(2)}}</view>
								<view class="gray9 f22">
									({{detail.advance.active_time[0]}}-{{detail.advance.active_time[1]}})</view>
							</view>
						</view>
						<view class="d-b-s">
			...
			<view class="bg-white p30 mb22" v-if="!ispresale">
				<view class="price-wrap">
					<view class="d-s-s d-c ww100">
						<view class="d-s-c pr ww100 mb16">
							<view class="new-price theme-price">
								<text class="" v-if="detail.isUserGrade">会员价</text>
								<text>¥</text>
								<text class="num">{{detail.productSku.productPrice }}</text><text
									v-if="detail.specType==20">-{{detail.highPrice}}</text>
							</view>
							<text class="old-price ">¥{{ detail.productSku.linePrice }}</text>
							<!--分享-->
							...
							<view class="sc-box">
								<button @click="favorite()">
									<image :class="isFav?'':'img_gray'" class="share_img" src="/static/icon/sc.png"
										mode=""></image>
									<text class="f22" :class="isFav?'red':'gray3'">收藏</text>
								</button>
							</view>
						</view>
						<view>
				...
				<view class="product-describe" v-if="detail.sellingPoint">{{ detail.sellingPoint }}</view>
			</view>
			<view class="o-h p-0-30 bg-white mb21 pb44" v-if="show_discount" @click="openCoupon()">
				<!--已选择-->
				<view class="already-choice">
					...
					<view class="d-b-c">
						<view class="flex-1">
							<view class="pb20 line-h-50 f22 gray3 " v-if="discount.give_points>0">
							...
							<view class="flex-1 d-b-c" v-if="discount.product_coupon.length > 0">
								<view class="f22 gray3 line-h-50 pb20 flex-1">
									<view class=" d-b-s">
										<text class="text-box ">领券</text>
										<view class="flex-1 text-coupon-list">
											<view class="ml20 mb10 text-coupon-box "
							...
							<view class="flex-1" v-if="discount.product_reduce.length > 0">
								<view class="f22 gray3 line-h-50 pb20">
			...
			<view class="o-h p-0-30 bg-white mb21">
				...
				<view class="already-choice" v-if="detail.spec_type == 20" @click="openPopup(ispresale?'deposit':'order')">
					<view class="group-hd">
						<view class="left">
							<text class="min-name gray3">选择：</text>
						</view>
			...
			<view class="product-comment">
				<view class="p-0-30 d-b-c">
					<view class="group-hd left">
						<text class="min-name f28">评价({{ commentDataCount }})</text>
					</view>
				...
				<view class="comment-list" v-if="commentDataCount > 0">
					<view class="item" v-if="index<=1" v-for="(item, index) in commentData" :key="index">
						<view class="cmt-user">
							<view class="left">
								<image class="photo" :src="item.avatarUrl" mode="aspectFill"></image>
			...
			<view class="product-content">
				<view class="p-0-30  border-b-e">
					<view class="group-hd d-s-c">
						<text class="min-name f28">商品介绍</text>
					</view>
				</view>
		...
		<view class="btns-wrap">
			<view class="icon-box d-c-c" @click="gotoPage('/pages/index/index')">
				<button class="d-c-c d-c bg-white">
					<text class="btn_btom pr icon iconfont icon-shouye gray9"
			...
			<view class="icon-box d-c-c">
				<button class="d-c-c d-c bg-white" open-type="contact" session-from="wxapp" show-message-card="true">
					<text class="icon iconfont icon-kefu gray9"
						style="font-size: 36rpx;height: 50rpx;line-height: 60rpx;"></text>
			...
			<view class="icon-box d-c-c" @click="gotocart">
				<button class="pr d-c-c d-c bg-white">
					<text class="gray9 icon iconfont icon-gouwuche1"
						style="font-size: 36rpx;height: 50rpx;line-height: 60rpx;"></text>
			...
			<template>
				<button v-if="!isVirtual&&!ispresale" class="add-cart" @click="openPopup('card')">加入购物车</button>
				...
				<button class="buy ispresale" v-else @click="openPopup('deposit')">
					...
					<view class="f22">￥{{detail.advance.money}}</view>
				</button>
			</template>
		</view>
