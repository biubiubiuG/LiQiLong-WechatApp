		...
		<view class="top-tabbar">
			<scroll-view scroll-x="true" scroll-with-animation="true">
				<view class="type-list d-s-c">
					<view :class="type_active == 0 ? 'tab-item  active' : 'tab-item '" @click="tabTypeFunc(0)">全部</view>
					<view
						:class="type_active == item.categoryId ? 'tab-item  active' : 'tab-item '"
						v-for="(item, index) in categorys"
						:key="index"
						@click="tabTypeFunc(item.categoryId)"
					>
						{{ item.name }}
					</view>
				</view>
		...
		<template>
			<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'" lower-threshold="50" @scrolltolower="scrolltolowerFunc">
				<view class="article-list">
					<view class="item" v-for="(item, index) in listData" :key="index" @click="gotoDetail(item.articleId)">
						<view class="info">
						...
						<view class="pic" v-if="item.filePath != null"><image :src="item.filePath" mode="aspectFill"></image></view>
					</view>
				</view>
